...
def Jarvis(points):
    ...
    while True:
        ...
        # set candidate for next point in hull
        q = (r0 + 1) % N

        for i in range(N):
            orient = geom.orientation(points[r0], points[i], points[q])

            # check if i is more clockwise than q, if yes set q = i
            if orient > 0 or (
                orient == 0
                and (
                    geom.distance(points[r0], points[i])
                    > geom.distance(points[r0], points[q])
                )
            ):
                q = i
